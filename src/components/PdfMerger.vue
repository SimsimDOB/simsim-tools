...
const validExtensionsString = () => {
  const upperExtensions = validExtensions.map(
...
<template>
  <div class="h-full bg-[#2e3440] text-[#d8dee9] p-8 font-['MakoP']">
    <div class="max-w-4xl mx-auto bg-[#3b4252] rounded-lg shadow-xl p-6">
      ...
      <div v-if="files.length > 0" class="mb-6">
        <h2 class="text-xl font-semibold text-[#88c0d0] mb-3">
          Selected Files ({{ files.length }})
        </h2>
        <ul class="space-y-2 max-h-96 overflow-y-auto pr-2 py-1">
          <li
            v-for="(file, index) in files"
            :key="file.id"
            @dragover.prevent="onDragOverItem(index)"
            @drop="onDropReorder(index)"
            class="relative flex items-center justify-between bg-[#434c5e] p-3 rounded border border-[#4c566a] transition-all duration-200"
          ...
          >
            ...
            <div class="flex items-center truncate flex-1 mr-4">
              <span class="text-[#88c0d0] font-mono mr-3">{{
                  index + 1
                }}.</span>
      ...
      <!-- Controls -->
      <div
        class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#434c5e] p-4 rounded-lg border border-[#4c566a]"
      >
        <div>
